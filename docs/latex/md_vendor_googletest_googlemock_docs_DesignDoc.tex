This page discusses the design of new Google \mbox{\hyperlink{classMock}{Mock}} features.\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md147}{}\doxysection{Macros for Defining Actions}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md147}
\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md148}{}\doxysubsection{Problem}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md148}
Due to the lack of closures in C++, it currently requires some non-\/trivial effort to define a custom action in Google \mbox{\hyperlink{classMock}{Mock}}. For example, suppose you want to \char`\"{}increment the value pointed to by the second argument of the mock function and return it\char`\"{}, you could write\+:
...
Our goal is to allow defining custom actions with the least amount of boiler-\/plate C++ requires.\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md149}{}\doxysubsection{Solution}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md149}
...
\end{longtabu}
\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md150}{}\doxysubsection{Parameterized actions}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md150}
...
We will also provide {\ttfamily ACTION\+\_\+\+P2}, {\ttfamily ACTION\+\_\+\+P3}, and etc to support multi-\/parameter actions. For example, 
\begin{DoxyCode}{0}
...
You can view {\ttfamily ACTION} as a degenerated parameterized action where the number of parameters is 0.\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md151}{}\doxysubsection{Advanced Usages}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md151}
\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md152}{}\doxysubsubsection{Overloading Actions}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md152}
...
\end{DoxyCode}
\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md153}{}\doxysubsubsection{Restricting the Type of an Argument or Parameter}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md153}
For maximum brevity and reusability, the {\ttfamily ACTION$\ast$} macros don\textquotesingle{}t let you specify the types of the mock function arguments and the action parameters. Instead, we let the compiler infer the types for us.
...
\end{DoxyCode}
 where {\ttfamily Static\+Assert\+Type\+Eq} is a compile-\/time assertion we plan to add to Google Test (the name is chosen to match {\ttfamily static\+\_\+assert} in C++0x).\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md154}{}\doxysubsubsection{Using the ACTION Object\textquotesingle{}s Type}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md154}
...
Note that we have to pick different suffixes ({\ttfamily Action}, {\ttfamily ActionP}, {\ttfamily Action\+P2}, and etc) for actions with different numbers of parameters, or the action definitions cannot be overloaded on the number of parameters.\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md155}{}\doxysubsection{When to Use}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md155}
While the new macros are very convenient, please also consider other means of implementing actions (e.\+g. via {\ttfamily Action\+Interface} or {\ttfamily Make\+Polymorphic\+Action()}), especially if you need to use the defined action a lot. While the other approaches require more work, they give you more control on the types of the mock function arguments and the action parameters, which in general leads to better compiler error messages that pay off in the long run. They also allow overloading actions based on parameter types, as opposed to just the number of parameters.\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md156}{}\doxysubsection{Related Work}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md156}
As you may have realized, the {\ttfamily ACTION$\ast$} macros resemble closures (also known as lambda expressions or anonymous functions). Indeed, both of them seek to lower the syntactic overhead for defining a function.
...
{\ttfamily ACTION$\ast$} avoid all these problems.\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md157}{}\doxysubsection{Future Improvements}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md157}
There may be a need for composing {\ttfamily ACTION$\ast$} definitions (i.\+e. invoking another {\ttfamily ACTION} inside the definition of one {\ttfamily ACTION$\ast$}). We are not sure we want it yet, as one can get a similar effect by putting {\ttfamily ACTION} definitions in function templates and composing the function templates. We\textquotesingle{}ll revisit this based on user feedback.

The reason we don\textquotesingle{}t allow {\ttfamily ACTION$\ast$()} inside a function body is that the current C++ standard doesn\textquotesingle{}t allow function-\/local types to be used to instantiate templates. The upcoming C++0x standard will lift this restriction. Once this feature is widely supported by compilers, we can revisit the implementation and add support for using {\ttfamily ACTION$\ast$()} inside a function.

C++0x will also support lambda expressions. When they become available, we may want to support using lambdas as actions.\hypertarget{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md158}{}\doxysection{Macros for Defining Matchers}\label{md_vendor_googletest_googlemock_docs_DesignDoc_autotoc_md158}
...
We will also add {\ttfamily MATCHER\+\_\+P}, {\ttfamily MATCHER\+\_\+\+P2}, and etc for parameterized matchers. 